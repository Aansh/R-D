...
bootloader_start:
	mov ax, 07C0h	; Set up 4K of stack space above buffer ;; Accumulator register (AX). Used in arithmetic operations. 
	add ax, 544		; 8k buffer = 512 paragraphs + 32 paragraphs (loader)
	cli				; Disable interrupts while changing stack
	mov ss, ax		;Stack Segment (SS). Pointer to the stack.
	mov sp, 4096	;Stack Pointer register (SP). Pointer to the top of the stack.
	sti				; Restore interrupts

	mov ax, 07C0h	; Set data segment to where we're loaded
	mov ds, ax		;Data Segment (DS). Pointer to the data.
